...
public class PongShaderExp1 : MonoBehaviour
{
    public Vector2  ballPosition = new Vector2(0,0);
    public float    ballSize = .08f;
    // public Vector2  paddle1Position = new Vector2(.4f,0);
    // public Vector2  paddle2Position = new Vector2(-.4f,0);
    public float    paddleWidth     = .1f;
    public float    paddleHeight    = .4f;
    public float    ballSpeed       = .02f;
    public float    paddleMoveSpeed = .5f;
 
    public Material pongMaterial;
    // private int     sleep = 0,
    //                 quit = 0,
    //                 state = 0,
    //                 r = 0;
    ...
    public struct Ball
    {
        public  float x, y, /* position on the screen */
                      w,h, // ball width and height
    ...
    public Paddle[] paddles = new Paddle[2]{new Paddle(),new Paddle()};

    void Start()
    {
        print(pongMaterial.GetVector("_ballPosition"));
        print(pongMaterial.GetFloat("_ballSize"));
        print(pongMaterial.GetVector("_paddle1Position"));
        print(pongMaterial.GetVector("_paddle2Position"));

        pongMaterial.SetVector("_ballPosition"   ,ballPosition);
        pongMaterial.SetFloat("_ballSize"        ,ballSize);
        pongMaterial.SetFloat("_ballSpeed"       ,ballSpeed);
        pongMaterial.SetFloat("_paddleWidth"     ,paddleWidth);
        pongMaterial.SetFloat("_paddleHeight"    ,paddleHeight);
    ...
    //Initialize starting position and game elemements
    void InitGame() 
    {
        ...
        ball.w  = ballSize;
        ball.h  = ballSize;
        Vector2 r = new Vector2(Random.Range(-1f,1f),Random.Range(-1f,1f)).normalized; // random direction
        ball.dx = r.x;
        ball.dy = r.y;
        ...
        paddles[1].x = -.4f;
        // paddles[1].y =    0;
        paddles[1].w = paddleWidth;
        paddles[1].h = paddleHeight;
    }
    ...
    void MoveBall() 
    {
        ...
        ball.x += ball.dx * Time.deltaTime * ballSpeed;
        ball.y += ball.dy * Time.deltaTime * ballSpeed;
        
        /* Turn the ball around if it hits the edge of the screen. */
        if (ball.x < -.5f - ball.w/2) 
            InitGame();

        if (ball.x > .5f + ball.w/2)
            InitGame();

        if (ball.y < -.5f + ball.h/2 || ball.y > .5f - ball.h/2)// - 10) 
            ball.dy = -ball.dy;

        //check for collision with the paddle
        for (int i = 0; i < 2; i++) 
        {
            int c = CheckCollision(ball, paddles[i]); 
            ...
            if (c == 1) 
            {
                ...
                // float hit_pos = (paddles[i].y + paddles[i].h) - ball.y;
                float hit_pos = paddles[i].y - ball.y;

                if (hit_pos <= .2f && hit_pos > .1f)
                {
                    print("hit low, hit pos: " + hit_pos);
                    ball.dy = 1;
                }
                if (hit_pos <= .1f && hit_pos > -.1f)
                {
                    print("hit center, hit pos: " + hit_pos);  
                    ball.dy = 0;
                }
                if (hit_pos <= -.1f && hit_pos > -.2f)
                { 
                    print("hit high, hit pos: " + hit_pos);
                    ball.dy = -1;
                }
    ...
    // void MovePaddleAI() 
    // {
    //     float center        = paddles[0].y + 25;
    //     float screen_center = .5f;
    //     float ball_speed    = ball.dy;
    ...
    //         {
    //             if (ball.y < center) 
    //                 paddles[0].y -= ball_speed * Time.deltaTime;
    //             else 
    //                 paddles[0].y += ball_speed * Time.deltaTime;
    //         }
    ...
    void MovePaddle(int d) 
    {
        print(d + " moving direction");
        // if the down arrow is pressed move paddle down
        if (d == 0) 
        {
            if(paddles[1].y >= .5f )//- paddles[1].h) 
                paddles[1].y = .5f ;//- paddles[1].h;
            else 
                paddles[1].y += Time.deltaTime * paddleMoveSpeed;
        }
    ...
    void DrawBall() 
    {
        pongMaterial.SetVector("_ballPosition", new Vector2(ball.x,ball.y));
    }

    void DrawPaddle() 
    {
        pongMaterial.SetVector("_paddle1Position",new Vector2(paddles[0].x,paddles[0].y));
        pongMaterial.SetVector("_paddle2Position",new Vector2(paddles[1].x,paddles[1].y));
    }

    void OnDisable() 
    {
        pongMaterial.SetVector("_ballPosition"   , new Vector2(0,0));
        pongMaterial.SetFloat("_ballSize"        ,ballSize);
        pongMaterial.SetFloat("_ballSpeed"       ,ballSpeed);
        pongMaterial.SetFloat("_paddleWidth"     ,paddleWidth);
        pongMaterial.SetFloat("_paddleHeight"    ,paddleHeight);   
        pongMaterial.SetVector("_paddle1Position",new Vector4(.4f,0));
        pongMaterial.SetVector("_paddle2Position",new Vector4(-.4f,0));
    }
}
